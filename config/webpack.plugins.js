...
const generateHTMLPlugins = () =>
  glob.sync('./src/template/pages/**/*.pug').map(dir => {
    const filename = path.basename(dir, '.pug');
...
const retinaImagesCopy = new CopyPlugin([
  {
    from: path.join(config.root, config.paths.src, 'retina'),
    to: 'retina/[name].[ext]'
  }
]);

const retinaImagesMin = new ImageminPlugin({
  test: /retina\/.*\.(jpe?g|png|gif|svg)$/i,
  gifsicle: {
    interlaced: false
  },
  optipng: {
...
module.exports = [
  ...
  config.env === 'production' && retinaImagesCopy,
  config.env === 'production' && retinaImagesMin,
  webpackBar,
